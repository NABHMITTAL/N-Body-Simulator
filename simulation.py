  ...
  def run(self):
    while self.renderer.state:
      ...
      if self.profile_frames == 100:
        print(f"\n========== Physics Function Profile {self.time_scales[self.time_scale_index]}x ==========")

        # print(f"distance()")
        # print(f"  Calls : {physics.distance_calls}")
        # print(f"  Avg   : {(physics.distance_time / physics.distance_calls) * 1e6:.3f} µs")

        print()

        print(f"displacement()")
        print(f"  Calls : {physics.displacement_calls}")
        print(f"  Avg   : {(physics.displacement_time / physics.displacement_calls) * 1e6:.3f} µs")

        print()

        print(f"gravitational_force()")
        print(f"  Calls : {physics.force_calls}")
        print(f"  Avg   : {(physics.force_time / physics.force_calls) * 1e6:.3f} µs")
